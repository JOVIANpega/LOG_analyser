...
def build_left_panel(parent, app):
    ...
    # 檔案選擇區域
    file_frame = ttk.LabelFrame(parent, text=" 📁 檔案來源 ", padding=(10, 10))
    file_frame.pack(fill=tk.X, padx=10, pady=5)
    
    # 智能選擇按鈕 (整合單檔/多檔/資料夾)
    btn_smart = ttk.Button(file_frame, text=" 📂 選擇 LOG 來源 (檔案/資料夾) ▼", 
                           bootstyle="info")
    ...
    # CSV檔案整理按鈕
    btn_csv = ttk.Button(file_frame, text=" 📊 CSV 檔案整理", 
                        command=app.start_csv_processing, bootstyle="success")
    ...
    # 清除結果按鈕
    btn_clear = ttk.Button(file_frame, text=" 🗑️ 清除結果", 
                          command=app._clear_enhanced_results, bootstyle="danger")
    ...
    btn_dir_manual = ttk.Button(isn_frame, text=" 📂 ", command=on_choose_manual_dir,
                               bootstyle="secondary-outline")
    btn_dir_manual.pack(side=tk.RIGHT, padx=(2, 0))
    app.left_btn_dir_manual = btn_dir_manual
    ...
    btn_search_img = ttk.Button(image_frame, text=" 🔍 開始檢索圖片 ", 
                               command=app._search_images_by_isn, bootstyle="primary")
    btn_search_img.pack(fill=tk.X, pady=5, ipady=3)
    app.left_btn_search_img = btn_search_img
    app.font_scaler.register(btn_search_img)
    _create_tooltip(btn_search_img, "搜尋 STATION_RECORD 資料夾中的圖片\n(JPG, PNG, YUV, BMP)", app=app)
    
    # 🟢 根據設定初始化狀態
    is_enabled = app.settings.get('enable_image_search', True)
    state_val = tk.NORMAL if is_enabled else tk.DISABLED
    isn_entry.config(state=state_val)
    btn_dir_manual.config(state=state_val)
    btn_search_img.config(state=state_val)
    if not is_enabled:
        image_frame.configure(text=" 🖼️ 圖片檢索 (已鎖定/LOCK) ")
    ...
    help_btn = ttk.Button(parent, text=" 📖 查看操作說明 (HTML)", 
                         command=app._open_html_help, bootstyle="secondary")
